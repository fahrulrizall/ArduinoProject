...
void krh(){
  ...
  float voltage1 = sensorValue * (5.0 / 1024.0)+0.4; // Convert the analog reading (which goes from 0 - 1023) to a voltage (0 - 5V):
  lcd.setCursor(0,2);
  lcd.print("k = ");
  lcd.print(voltage1);
...
void ph() {
  //cek kondisi ph
   ...
   lcd.print(pHValue);
   //penentuan aksi pH
   if(pHValue < minph && voltage1 < keruh ){ 
   ...
   if(pHValue >= minph && pHValue <= maxph && voltage1 >= keruh ){
   digitalWrite(RELAY_3, RELAY_ON);
   digitalWrite(31, LOW);
   digitalWrite(30, HIGH); 
   lcd.setCursor(10,3);
